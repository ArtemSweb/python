...
lang = input('Какой язык будем применять? ru / en: ')
direction = input('Что вы ходите сделать? Шифрование или дешифрование? Введите encrypt или decrypt: ')
...
# Алгоритм шифрования
def ceasar_encrypt(text, alphabet, step):
    result_text = []
    for letter in text:
...
if is_valid_input(direction, lang):
    if direction == 'encrypt':
        if lang == 'en':
            print(ceasar_encrypt(original_text, en_alphabet, step))
        else:
            print(ceasar_encrypt(original_text, ru_alphabet, step))
    elif direction == 'decrypt':
        if lang == 'en':
            if question == 'yes':
                print(ceasar_encrypt(original_text, en_alphabet, -(int(step))))
            else:
                for i in range(len(en_alphabet)):
                    print('Вариант', i, ceasar_encrypt(original_text, en_alphabet, i))
        elif lang == 'ru':
            if question == 'yes':
                print(ceasar_encrypt(original_text, ru_alphabet, -(int(step))))
            else:
                for i in range(len(ru_alphabet)):
                    print('Вариант', i, ceasar_encrypt(original_text, ru_alphabet, i))                          
else:
